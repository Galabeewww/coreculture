 ...
 *
 * Fitur:
 * - Dropdown "SHOP" menampilkan daftar kategori produk (dengan pesan "Belum ada kategori" jika kosong).
 * - Dropdown "COLLECTIONS" menampilkan daftar koleksi eksklusif (dengan pesan "Belum ada koleksi" jika kosong).
 * - Menangani error API dengan aman (Array.isArray) untuk mencegah runtime error.
 */
        ...
        <div className="flex h-20 items-center justify-between">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              {/* SALE! */}
              <Link href="/#katalog" className="text-xs font-black tracking-widest text-blue-800 hover:text-blue-300 transition-colors duration-200 uppercase">
                SALE!
              </Link>
              ...
              <div ref={shopRef} className="relative">
                ...
                >
                  SHOP <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${shopOpen ? "rotate-180" : ""}`} />
                </button>
                {shopOpen && (
                  <div className="absolute top-full left-0 mt-3 w-56 bg-white border border-zinc-200 rounded-lg shadow-xl py-2 animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                    ...
                    {loadingCategories ? (
                      ...
                      categories.map((cat) => (
                        <Link
                          ...
                          className="block px-4 py-2.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 hover:text-primary transition-colors"
                        >
                          {cat.name}
                        </Link>
                      ))
              ...
              <div ref={colRef} className="relative">
                <button
                  ...
                  className="flex items-center gap-1 text-xs font-semibold tracking-widest text-zinc-600 hover:text-primary transition-colors duration-200 uppercase cursor-pointer"
                >
                  COLLECTIONS <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${collectionsOpen ? "rotate-180" : ""}`} />
                </button>
                {collectionsOpen && (
                  <div className="absolute top-full left-0 mt-3 w-64 bg-white border border-zinc-200 rounded-lg shadow-xl py-2 animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                    <div className="px-4 py-2 border-b border-zinc-100">
